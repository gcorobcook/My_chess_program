...
class GUIgame(chess.game):
    def __init__(self,FEN=None,history=None,pos_history=None):
        ...
        pygame.init()
        super().__init__(FEN,history,pos_history)
    ...
    def pos2pic(self):
        ...
        for i in my_range:
            for j in my_range:
                target = self.FEN.position[7-i,j,:12]
                target = np.nonzero(target)[0]
                if target.size > 0:
                    target = target[0]
                    piece = pygame.image.load(os.path.join(this_dir,"Images",self.pieces[target]+".png"))
                    piece = pygame.transform.scale(piece,(self.cs,self.cs))
                    if self.play_option == 'twopl' and target>=6:
    ...
    def clicks2move(self,event,move):
        ...
        if move[4] != None:
            if event.type == pygame.MOUSEBUTTONDOWN:
                if click != move[3:5]:
                    ...
                    if (self.FEN.position[click[0],click[1],:12] != 0).any():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if click == None:
                return [None]*6
            if move[0] == None:
                target = self.FEN.position[click[0],click[1],:12]
                target = np.nonzero(target)[0]
                if target.size > 0 and (target[0]<6) == self.FEN.colour():
                    move[0] = target[0]
                    move[1],move[2] = click
                    return move
                else:
        ...
        for valid in self.FEN.moves():
            if move[:5] == list(valid)[:5]:
                if valid[5] != '' and valid[5].lower() in 'qrbn':
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            return self.clicks2move(event,[None]*6)
        else:
            return [None]*6
    ...
    def timevsinsufficient(self):
        ...
        if self.FEN.colour():
            piece_range = range(1,6)
        else:
            piece_range = range(7,12)
        for i in range(8):
            for j in range(8):
                target = self.FEN.position[i,j,:12]
                target = np.nonzero(target)[0]
                if target.size > 0 and target[0] in piece_range:
                    return False
        return True
        ...
        while mainloop:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if not None in move:
                        return move
                    elif move[4] != None: #pawn promotions
                        #show clickable piece pictures
                        pieces = ['queen','rook','bishop','knight']
                        if self.player:
                            cell_x = self.board_x + move[4]*self.cs
                            cell_y = self.board_y + (7-move[3])*self.cs
                        else:
                            cell_x = self.board_x + move[4]*self.cs
                            cell_y = self.board_y + move[3]*self.cs
                        for i in range(4):
                            if self.play_option == 'twopl' and not self.FEN.colour():
                                piece = pieces[3-i]
                            else:
                            ...
                            if self.FEN.colour():
                                ...
                                piece += '_black.png'
                            piece = pygame.image.load(os.path.join(this_dir,"Images",piece))
    ...
    def playing(self):
        ...
        if self.play_option in ['play_w','play_b']:
            if os.path.exists("best_model.keras"):
                model = load_model("best_model.keras")
            else:
                model = chessAI.build_model('best_model')
            tree = chessAI.MCTS_tree()
        ...
        while mainloop:
            if self.play_option == 'play_w':
                if self.FEN.colour():
                    ...
                    tree.build(self.FEN,model,False)
                    root = tree[self.FEN.FEN_string()]
                    probs = root.N
            ...
            if event.type == pygame.QUIT:
                ...
                if self.copy_button_x < x and x < self.copy_button_x + 2*self.cs:
                    if self.copy_button1_y < y and y < self.copy_button2_y:
                        ...
                        r.withdraw()
                        r.clipboard_clear()
                        r.clipboard_append(self.FEN.FEN_string())
                        r.update()
